...
import static com.hb.xtvfileexplorer.loader.DirectoryLoader.SORT_ORDER_DISPLAY_NAME;
import static com.hb.xtvfileexplorer.loader.DirectoryLoader.SORT_ORDER_LAST_MODIFIED;
import static com.hb.xtvfileexplorer.loader.DirectoryLoader.SORT_ORDER_SIZE;
...
public class SortingCursorWrapper extends AbstractCursor {
    ...
    public SortingCursorWrapper(Cursor cursor, int sortOrder) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch (sortOrder) {
                case SORT_ORDER_DISPLAY_NAME:
                    final String mimeType = RootInfo.getCursorString(cursor, Document.COLUMN_MIME_TYPE);
                    final String displayName = RootInfo.getCursorString(
                    ...
                    break;
                case SORT_ORDER_LAST_MODIFIED:
                    mValueLong[i] = RootInfo.getCursorLong(cursor, Document.COLUMN_LAST_MODIFIED);
                    break;
                case SORT_ORDER_SIZE:
                    mValueLong[i] = RootInfo.getCursorLong(cursor, Document.COLUMN_SIZE);
                    break;
            }
        }
